...
class Lap_LWCC_Spectra(baseWithHistPlot):
    def __init__(self,G1, optionsDict):
        G = G1.subgraph(max(nx.weakly_connected_components(G1), key=len))
...
class RwLap_LWCC_Spectra(baseWithHistPlot):
    def __init__(self,G1, optionsDict):
        G = G1.subgraph(max(nx.weakly_connected_components(G1), key=len))
        A0 = nx.to_scipy_sparse_matrix(G)
        A = (A0 + A0.T)/2
...
class Lap_Spectra_NoWeight(baseWithHistPlot):
    def __init__(self,G1, optionsDict):
        G = G1.subgraph(max(nx.weakly_connected_components(G1), key=len))
...
class RwLap_Spectra_NoWeight(baseWithHistPlot):
    def __init__(self,G1, optionsDict):
        G = G1.subgraph(max(nx.weakly_connected_components(G1), key=len))
        A0 = nx.to_scipy_sparse_matrix(G,weight=None)
        A = (A0 + A0.T)/2
